" Vim syntax file
" Language:	Scala
" Maintainer:	Derek Whyatt
" URL:          https://github.com/derekwyatt/vim-scala
" License:      Apache 2

" Modifyed:     Nikolay Konovalow <major.kolz@gmail.com>
" ----------------------------------------------------------------------------

" - Clear all syntax items for old versions of vim
...
" Case is significant for syntax highlighting 
syn case match
" Prevent highlighting's color from mess when scrolling.  
syn sync minlines=5 maxlines=75

" === Comments
syn keyword scalaMarker FIXME TODO XXX contained 
syn match scalaTrailingComment "//.*$" contains=scalaMarker,@Spell
syn region scalaMultilineComment start="/\*" end="\*/" contains=scalaDocLinks,scalaCommentAnnotation,scalaParameterAnnotation,scalaCommentCodeBlock,@scalaHtml, @Spell keepend
...
" === Conditionals
syn keyword scalaCond do else for forSome if while
hi link scalaCond Conditional


" === Types
syn keyword scalaType val var
hi link scalaType Type


" === String
syn match scalaStringEmbeddedQuote /\\"/ contained
syn region scalaString start=/"/ end=/"/ contains=scalaStringEmbeddedQuote,scalaEscapedChar,scalaUnicodeChar,@Spell
...
syn keyword scalaKeyword catch final finally
syn keyword scalaKeyword match return throw try yield macro
syn keyword scalaKeyword class trait object extends with nextgroup=scalaInstanceDeclaration skipwhite
syn keyword scalaKeyword case nextgroup=scalaKeyword,scalaCaseFollowing skipwhite
syn keyword scalaKeyword nextgroup=scalaNameDefinition,scalaQuasiQuotes skipwhite
syn keyword scalaKeyword def nextgroup=scalaNameDefinition,scalaTheMainFunc skipwhite
...
" def main will be highlighting by the same color - so, it visually separated from other methods
syn match scalaTheMainFunc /\<main\>/ contained nextgroup=scalaPostNameDefinition
syn match scalaNameDefinition /\<\(main\)\@![_A-Za-z0-9$]\+\>/ contained nextgroup=scalaPostNameDefinition
syn match scalaNameDefinition /`[^`]\+`/ contained nextgroup=scalaPostNameDefinition
syn match scalaPostNameDefinition /\_s*:\_s*/ contained nextgroup=scalaTypeDeclaration
hi link scalaTheMainFunc scalaKeyword
